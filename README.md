...
- **Алерты**: пороги warning/critical/recovery для CPU, RAM, swap, дисков, RTT и времени ответа HTTP; параметр `for` — минимальная длительность превышения порога перед отправкой оповещения (короткие пики не алертят)
