...
| Секция | Описание |
|--------|----------|
| `telegram` | токен бота, `allowed_users` (первый ID всегда root), `db_path` — каталог SQLite с ролями и хостами (пусто = `{каталог конфига}/db`), `delete_after` — автоудаление сообщений бота (по умолчанию `48h`, максимум `48h`, `0s` — выключить) |
...
Бот периодически удаляет **свои** сообщения старше `telegram.delete_after` (по умолчанию 48 часов — максимум, который позволяет Telegram Bot API в личных чатах). Сообщения, отправленные до обновления, не затрагиваются.
